  ...
  <div class="row">
    ...
    <div class="col-md-9">
      ...
      if (isset($_POST['update'])) {
        ...
        $query = "SELECT `password` FROM customer Where cust_id=$customer_id";
          ...
          if (!empty($old_pass) && !empty($new_pass) && !empty($confirm_pass)) {
            if ($old_pass === $cust_pass) {
              if ($new_pass === $confirm_pass) {
                ...
                $up_query = "UPDATE customer SET password = '$confirm_pass'";
                ...
                $error = "<div class='alert alert-danger alert-dismissible fade show pt-1 pb-1 pl-3'  role='alert'>
                            Mật khẩu mới và mật khẩu xác nhận phải trùng khớp!
                           <button type='button' class='close p-2' data-dismiss='alert' aria-label='Close'>
                            <span  aria-hidden='true'>&times;</span>
                           </button>
                            </div>";
