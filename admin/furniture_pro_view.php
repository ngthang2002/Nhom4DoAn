...
if (isset($_GET['del'])) {
  $del   = $_GET['del'];
  $query = "DELETE FROM furniture_product WHERE id = $del";
      ...
      <table class="table table-responsive table-hover ">
        <thead class="thead-light">
          <tr>
            ...
            <th>Trạng thái</th>
            <th>Ngày</th>
            <th colspan="4">Thao tác(Sửa/Xóa)</th>
            <th colspan="4"></th>
          </tr>
        </thead>
        <tbody>
          ...
          if (isset($status)) {
            $pr_query = "SELECT * FROM categories cat INNER JOIN furniture_product fp ON fp.category = cat.id WHERE fp.status = '$status'";
          } else {
            $pr_query = "SELECT * FROM categories cat INNER JOIN furniture_product fp ON fp.category = cat.id order by fp.id limit 15";
          }
